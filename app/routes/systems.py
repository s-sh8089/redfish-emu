...
@router.get('/redfish/v1/Systems/system')
@router.get('/redfish/v1/Systems/system/')
def system_get(db: sqlite3.Connection = Depends(get_db)):
    row = db.execute('SELECT * FROM systems WHERE id="system"').fetchone()
    if not row:
        return not_found_response()
    return _system_response(db, row)


@router.patch('/redfish/v1/Systems/system')
@router.patch('/redfish/v1/Systems/system/')
def system_patch(
    ...
    if 'BootSourceOverrideTarget' in boot:
        fields.append('boot_source_override_target=?'); values.append(boot['BootSourceOverrideTarget'])
    if 'BootSourceOverrideEnabled' in boot:
        fields.append('boot_source_override_enabled=?'); values.append(boot['BootSourceOverrideEnabled'])
    if 'BootSourceOverrideMode' in boot:
        fields.append('boot_source_override_mode=?'); values.append(boot['BootSourceOverrideMode'])
    if 'PowerRestorePolicy' in data:
        fields.append('power_restore_policy=?'); values.append(data['PowerRestorePolicy'])
    if 'AssetTag' in data:
        fields.append('asset_tag=?'); values.append(data['AssetTag'])
    if 'PowerMode' in data:
        fields.append('power_mode=?'); values.append(data['PowerMode'])
    if fields:
...
def _system_response(db, row):
    ...
    return json_response({
        ...
        'ProcessorSummary': {'Count': procs, 'Status': {'Health': 'OK', 'State': 'Enabled'}},
        'MemorySummary': {
...
@router.post('/redfish/v1/Systems/system/Actions/ComputerSystem.Reset')
def system_reset(
    body: dict | None = Body(default=None),
    db: sqlite3.Connection = Depends(get_db),
):
...
@router.get('/redfish/v1/Systems/system/SecureBoot/')
def secure_boot_get(db: sqlite3.Connection = Depends(get_db)):
    row = db.execute('SELECT secure_boot_enable FROM systems WHERE id="system"').fetchone()
    if not row:
        return not_found_response()
    return _secure_boot_response(row)


@router.patch('/redfish/v1/Systems/system/SecureBoot/')
def secure_boot_patch(
    body: dict | None = Body(default=None),
    db: sqlite3.Connection = Depends(get_db),
):
    row = db.execute('SELECT secure_boot_enable FROM systems WHERE id="system"').fetchone()
...
@router.get('/redfish/v1/Systems/system/Bios/')
def bios_get(db: sqlite3.Connection = Depends(get_db)):
    ...
    attrs = json.loads(row['bios_attributes']) if row['bios_attributes'] else {
        'BootMode': 'Uefi', 'NicBoot1': 'NetworkBoot', 'NicBoot2': 'Disabled',
        'QuietBoot': True, 'SriovGlobalEnable': 'Disabled',
    }
    return _bios_response(attrs)


@router.patch('/redfish/v1/Systems/system/Bios/')
def bios_patch(
    body: dict | None = Body(default=None),
    db: sqlite3.Connection = Depends(get_db),
):
    ...
    if 'Attributes' in data and isinstance(data['Attributes'], dict):
        ...
        db.execute('UPDATE systems SET bios_attributes=? WHERE id="system"', (json.dumps(attrs),))
        db.commit()
    return _bios_response(attrs)


def _bios_response(attrs):
    return json_response({
...
@router.post('/redfish/v1/Systems/system/Bios/Actions/Bios.ResetBios')
def bios_reset(db: sqlite3.Connection = Depends(get_db)):
    db.execute('UPDATE systems SET bios_attributes=NULL WHERE id="system"')
    db.commit()
    return no_content_response()


@router.post('/redfish/v1/Systems/system/Bios/Actions/Bios.ChangePassword')
def bios_change_password(body: dict | None = Body(default=None)):
    data = body or {}
...
@router.get('/redfish/v1/Systems/system/Processors/')
def processors(db: sqlite3.Connection = Depends(get_db)):
    rows = db.execute('SELECT id FROM processors WHERE system_id="system"').fetchall()
    members = [{'@odata.id': f'{BASE}/system/Processors/{row["id"]}/'} for row in rows]
    return json_response({
...
@router.get('/redfish/v1/Systems/system/Processors/{proc_id}/')
def processor(proc_id: str, db: sqlite3.Connection = Depends(get_db)):
...
@router.get('/redfish/v1/Systems/system/Memory/')
def memory_collection(db: sqlite3.Connection = Depends(get_db)):
    rows = db.execute('SELECT id FROM memory_modules WHERE system_id="system"').fetchall()
    members = [{'@odata.id': f'{BASE}/system/Memory/{row["id"]}/'} for row in rows]
    return json_response({
...
@router.get('/redfish/v1/Systems/system/Memory/{mem_id}/')
def memory(mem_id: str, db: sqlite3.Connection = Depends(get_db)):
...
@router.get('/redfish/v1/Systems/system/Memory/{mem_id}/MemoryMetrics/')
def memory_metrics(mem_id: str, db: sqlite3.Connection = Depends(get_db)):
    if not db.execute('SELECT id FROM memory_modules WHERE id=? AND system_id="system"', (mem_id,)).fetchone():
        return not_found_response()
    return json_response({
        '@odata.id': f'{BASE}/system/Memory/{mem_id}/MemoryMetrics/',
        '@odata.type': '#MemoryMetrics.v1_7_1.MemoryMetrics',
        'Id': 'MemoryMetrics', 'Name': 'Memory Metrics', 'Description': 'Memory Metrics',
        'HealthData': {'AlarmTrips': {
            'Temperature': False, 'SpareBlock': False, 'UncorrectableECCError': False,
            'CorrectableECCError': False, 'AddressParityError': False
        }}
    })


@router.get('/redfish/v1/Systems/system/Storage/')
def storage_collection(db: sqlite3.Connection = Depends(get_db)):
    rows = db.execute('SELECT id FROM storage WHERE system_id="system"').fetchall()
    members = [{'@odata.id': f'{BASE}/system/Storage/{row["id"]}/'} for row in rows]
    return json_response({
...
@router.get('/redfish/v1/Systems/system/Storage/{storage_id}/')
def storage(storage_id: str, db: sqlite3.Connection = Depends(get_db)):
    ...
    drive_members = [{'@odata.id': f'{BASE}/system/Storage/{storage_id}/Drive/{d["id"]}/'} for d in drives]
    return json_response({
...
@router.get('/redfish/v1/Systems/system/Storage/{storage_id}/Controllers/')
def storage_controllers(storage_id: str, db: sqlite3.Connection = Depends(get_db)):
...
@router.get('/redfish/v1/Systems/system/Storage/{storage_id}/Controllers/{ctrl_id}/')
def storage_controller(storage_id: str, ctrl_id: str, db: sqlite3.Connection = Depends(get_db)):
    if not db.execute('SELECT id FROM storage WHERE id=? AND system_id="system"', (storage_id,)).fetchone():
        return not_found_response()
    if ctrl_id != '0':
        return not_found_response()
    return json_response({
        ...
        'Id': ctrl_id, 'Name': 'Storage Controller 0',
        'Manufacturer': 'Dell Inc.', 'Model': 'PERC H755', 'FirmwareVersion': '1.0.0',
...
@router.get('/redfish/v1/Systems/system/Storage/{storage_id}/Volumes/')
def volumes(storage_id: str, db: sqlite3.Connection = Depends(get_db)):
    if not db.execute('SELECT id FROM storage WHERE id=? AND system_id="system"', (storage_id,)).fetchone():
        return not_found_response()
    return json_response({
...
@router.get('/redfish/v1/Systems/system/Storage/{storage_id}/Drive/{drive_id}/')
def drive(storage_id: str, drive_id: str, db: sqlite3.Connection = Depends(get_db)):
    ...
    return json_response({
        ...
        'Status': {'State': row['status_state'], 'Health': row['status_health']},
        'Links': {'Chassis': {'@odata.id': '/redfish/v1/Chassis/chassis1/'}}
    })


@router.get('/redfish/v1/Systems/system/EthernetInterfaces/')
def system_ethernet_interfaces(db: sqlite3.Connection = Depends(get_db)):
    rows = db.execute("SELECT id FROM ethernet_interfaces WHERE parent_type='system'").fetchall()
    members = [{'@odata.id': f'{BASE}/system/EthernetInterfaces/{row["id"]}/'} for row in rows]
    return json_response({
...
@router.get('/redfish/v1/Systems/system/EthernetInterfaces/{iface_id}/')
def system_ethernet_interface(iface_id: str, db: sqlite3.Connection = Depends(get_db)):
    row = db.execute(
        "SELECT * FROM ethernet_interfaces WHERE id=? AND parent_type='system'", (iface_id,)
    ).fetchone()
    if not row:
        return not_found_response()
    ...
    return json_response({
        ...
        'IPv6Addresses': ipv6, 'NameServers': dns,
        'Status': {'State': row['status_state'], 'Health': row['status_health']}
    })


@router.get('/redfish/v1/Systems/system/FabricAdapters/')
def fabric_adapters(db: sqlite3.Connection = Depends(get_db)):
    rows = db.execute('SELECT id FROM fabric_adapters WHERE system_id="system"').fetchall()
    members = [{'@odata.id': f'{BASE}/system/FabricAdapters/{row["id"]}/'} for row in rows]
    return json_response({
...
@router.get('/redfish/v1/Systems/system/FabricAdapters/{adapter_id}/')
def fabric_adapter(adapter_id: str, db: sqlite3.Connection = Depends(get_db)):
    ...
    return json_response({
        ...
        'Id': row['id'], 'Name': row['id'],
        'Model': row['model'], 'PartNumber': row['part_number'],
        'SerialNumber': row['serial_number'], 'SparePartNumber': row['spare_part_number'],
        'Status': {'State': row['status_state'], 'Health': row['status_health']},
        'Location': {}
    })


@router.get('/redfish/v1/Systems/system/PCIeDevices/')
def pcie_devices(db: sqlite3.Connection = Depends(get_db)):
    rows = db.execute('SELECT id FROM pcie_devices WHERE system_id="system"').fetchall()
    members = [{'@odata.id': f'{BASE}/system/PCIeDevices/{row["id"]}/'} for row in rows]
    return json_response({
...
@router.get('/redfish/v1/Systems/system/PCIeDevices/{device_id}/')
def pcie_device(device_id: str, db: sqlite3.Connection = Depends(get_db)):
    ...
    return json_response({
        ...
        'Id': row['id'], 'Name': row['id'],
        'PCIeInterface': {'LanesInUse': row['lanes_in_use']}
    })


@router.get('/redfish/v1/Systems/system/LogServices/')
...
@router.get('/redfish/v1/Systems/system/LogServices/EventLog/')
def system_eventlog():
    return json_response({
        '@odata.id': f'{BASE}/system/LogServices/EventLog/',
        '@odata.type': '#LogService.v1_4_0.LogService',
        'Id': 'EventLog', 'Name': 'Event Log', 'Description': 'System Event Log',
...
@router.post('/redfish/v1/Systems/system/LogServices/EventLog/Actions/LogService.ClearLog')
def system_eventlog_clear(db: sqlite3.Connection = Depends(get_db)):
    db.execute("DELETE FROM log_entries WHERE log_service_id='EventLog' AND parent_type='system'")
    db.commit()
    return no_content_response()


@router.get('/redfish/v1/Systems/system/LogServices/EventLog/Entries/')
def system_eventlog_entries(db: sqlite3.Connection = Depends(get_db)):
    rows = db.execute(
        "SELECT * FROM log_entries WHERE log_service_id='EventLog' AND parent_type='system'"
    ).fetchall()
    members = [{'@odata.id': f'{BASE}/system/LogServices/EventLog/Entries/{row["id"]}/'} for row in rows]
    return json_response({
...
@router.post('/redfish/v1/Systems/system/LogServices/EventLog/Entries/')
def system_eventlog_entries_post(
    body: dict | None = Body(default=None),
    db: sqlite3.Connection = Depends(get_db),
):
    return _create_log_entry(db, 'EventLog', 'system',
                             f'{BASE}/system/LogServices/EventLog/Entries/', body or {})


@router.get('/redfish/v1/Systems/system/LogServices/EventLog/Entries/{entry_id}/')
def system_eventlog_entry_get(entry_id: str, db: sqlite3.Connection = Depends(get_db)):
    row = db.execute(
    ...
    if not row:
        return not_found_response()
    return json_response(log_entry_to_dict(row, f'{BASE}/system/LogServices/EventLog/Entries/{entry_id}/'))


@router.patch('/redfish/v1/Systems/system/LogServices/EventLog/Entries/{entry_id}/')
def system_eventlog_entry_patch(
    entry_id: str,
    body: dict | None = Body(default=None),
    db: sqlite3.Connection = Depends(get_db),
):
    ...
    if not db.execute(
        "SELECT id FROM log_entries WHERE id=? AND log_service_id='EventLog' AND parent_type='system'",
        (entry_id,)
    ).fetchone():
        return not_found_response()
...
@router.delete('/redfish/v1/Systems/system/LogServices/EventLog/Entries/{entry_id}/')
def system_eventlog_entry_delete(entry_id: str, db: sqlite3.Connection = Depends(get_db)):
    if not db.execute(
        "SELECT id FROM log_entries WHERE id=? AND log_service_id='EventLog' AND parent_type='system'",
        (entry_id,)
    ).fetchone():
...
@router.get('/redfish/v1/Systems/system/LogServices/SEL/')
def system_sel():
    return json_response({
        '@odata.id': f'{BASE}/system/LogServices/SEL/',
        '@odata.type': '#LogService.v1_4_0.LogService',
        'Id': 'SEL', 'Name': 'SEL', 'Description': 'IPMI System Event Log',
        'DateTime': now_iso(), 'DateTimeLocalOffset': '+00:00',
        'MaxNumberOfRecords': 4096, 'OverWritePolicy': 'WrapsWhenFull',
        'Status': {'State': 'Enabled', 'Health': 'OK'},
...
@router.post('/redfish/v1/Systems/system/LogServices/SEL/Actions/LogService.ClearLog')
def system_sel_clear(db: sqlite3.Connection = Depends(get_db)):
    db.execute("DELETE FROM log_entries WHERE log_service_id='SEL' AND parent_type='system'")
    db.commit()
    return no_content_response()


@router.get('/redfish/v1/Systems/system/LogServices/SEL/Entries/')
def system_sel_entries(db: sqlite3.Connection = Depends(get_db)):
    rows = db.execute(
        "SELECT * FROM log_entries WHERE log_service_id='SEL' AND parent_type='system'"
    ).fetchall()
    members = [{'@odata.id': f'{BASE}/system/LogServices/SEL/Entries/{row["id"]}/'} for row in rows]
    return json_response({
        '@odata.id': f'{BASE}/system/LogServices/SEL/Entries/',
        '@odata.type': '#LogEntryCollection.LogEntryCollection',
        'Name': 'Log Entry Collection', 'Description': 'SEL entries',
        'Members@odata.count': len(members), 'Members': members
    })


@router.post('/redfish/v1/Systems/system/LogServices/SEL/Entries/')
def system_sel_entries_post(
    body: dict | None = Body(default=None),
    db: sqlite3.Connection = Depends(get_db),
):
    return _create_log_entry(db, 'SEL', 'system',
                             f'{BASE}/system/LogServices/SEL/Entries/', body or {})


@router.get('/redfish/v1/Systems/system/LogServices/SEL/Entries/{entry_id}/')
def system_sel_entry_get(entry_id: str, db: sqlite3.Connection = Depends(get_db)):
    row = db.execute(
    ...
    if not row:
        return not_found_response()
    return json_response(log_entry_to_dict(row, f'{BASE}/system/LogServices/SEL/Entries/{entry_id}/'))


@router.patch('/redfish/v1/Systems/system/LogServices/SEL/Entries/{entry_id}/')
def system_sel_entry_patch(
    entry_id: str,
    body: dict | None = Body(default=None),
    db: sqlite3.Connection = Depends(get_db),
):
    ...
    if not db.execute(
        "SELECT id FROM log_entries WHERE id=? AND log_service_id='SEL' AND parent_type='system'",
        (entry_id,)
    ).fetchone():
        return not_found_response()
    return _patch_log_entry(db, entry_id, odata_id, body or {})
...
@router.delete('/redfish/v1/Systems/system/LogServices/SEL/Entries/{entry_id}/')
def system_sel_entry_delete(entry_id: str, db: sqlite3.Connection = Depends(get_db)):
    if not db.execute(
        "SELECT id FROM log_entries WHERE id=? AND log_service_id='SEL' AND parent_type='system'",
        (entry_id,)
    ).fetchone():
