...
def create_task(db: sqlite3.Connection, messages=None) -> str:
...
def complete_task(db: sqlite3.Connection, task_id: str, messages=None, status: str = 'OK'):
    now = datetime.now(timezone.utc).isoformat()
    db.execute(
...
def tasks(db: sqlite3.Connection = Depends(get_db)):
...
def task_get(task_id: str, db: sqlite3.Connection = Depends(get_db)):
...
def task_delete(task_id: str, db: sqlite3.Connection = Depends(get_db)):
